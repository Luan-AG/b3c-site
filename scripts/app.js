...
//------------------MENU OVERLAY EFFECT ON MOBILE---------------------------------
{
  ...
  const elementToShow = document.querySelector(".lang-menu-mobile ul");
  const btnOverlay = document.querySelectorAll('.botao-overlay');
  const btnMenuOpen = document.querySelector('.btn-menu-mobile');
  const btnMenuClose = document.querySelector('.closebtn');
  
  btnMenuOpen.addEventListener('click', openNav);
  btnMenuClose.addEventListener('click', closeNav);
